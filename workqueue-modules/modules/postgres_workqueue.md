...
PostgreSQL-native implementation of `workqueue.WorkQueue`. It shares [`gorm_workqueue`](./gorm_workqueue.md)'s table layout but exploits PostgreSQL-only features for lower latency and less database pressure:
...
Requires `postgres_connector` declared **before** it in the same fx graph:
