...
Database-backed implementation of `workqueue.WorkQueue` on top of any `database.DatabaseConnector` from common-modules (PostgreSQL, SQLite, ...). Tasks are durable rows; consumers poll for ready tasks and claim them with an optimistic status transition, so multiple processes can safely compete for the same queue.
...
Requires a `database.DatabaseConnector` (e.g. `sqlite_connector` or `postgres_connector`) declared **before** it in the same fx graph:
...
- **Cross-process pickup latency** is bounded by `poll_interval`; tasks scheduled locally (delays, retries) fire on time regardless. On PostgreSQL, [`postgres_workqueue`](./postgres_workqueue.md) removes this bound via LISTEN/NOTIFY.
- **SQLite works well** for single-process apps: enqueues, retries, and scheduled tasks all ride on in-process wakeups, so the poll interval never drives latency. There is no dedicated SQLite backend — this module is the recommended choice.
- **Same table layout as `postgres_workqueue`** — you can start here and switch modules later without migrating data.
