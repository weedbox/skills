...
`sqlite_connector` and `postgres_connector` can be loaded into the same `fx.App` — for example to run SQLite for a local cache alongside a PostgreSQL primary. Each `Module(scope)` always registers a named connector keyed by its `scope`; consumers disambiguate via fx's `name` tag:
...
```bash
export MYAPP_DATABASE_PATH=./data/app.db
export MYAPP_DATABASE_LOGLEVEL=2
export MYAPP_DATABASE_DEBUG_MODE=false
export MYAPP_DATABASE_ENABLE_WAL=true
export MYAPP_DATABASE_BUSY_TIMEOUT=5000
export MYAPP_DATABASE_MAX_OPEN_CONNS=10
export MYAPP_DATABASE_MAX_IDLE_CONNS=1
export MYAPP_DATABASE_CONN_MAX_LIFETIME=3600
export MYAPP_DATABASE_CONN_MAX_IDLE_TIME=30
export MYAPP_DATABASE_ENABLE_READ_WRITE_SPLIT=true
export MYAPP_DATABASE_WRITE_MAX_OPEN_CONNS=10
export MYAPP_DATABASE_WRITE_MAX_IDLE_CONNS=1
export MYAPP_DATABASE_SYNCHRONOUS=NORMAL
export MYAPP_DATABASE_CACHE_SIZE=-65536
export MYAPP_DATABASE_LOCKING_MODE=
```
