...
- **Subjects drift reconciliation (`EnsureStream` only)** — on every lookup hit, the existing stream's bound `Subjects` are compared against `cfg.Subjects` as a set (order-independent). On drift, an idempotent `CreateOrUpdateStream` is issued that overlays **only `Subjects`** on top of the existing server-side config, preserving `Replicas` / `MaxBytes` / `MaxAge` so an existing R=1 stream under a cfg requesting R=3 does not death-loop on insufficient peers during reconciliation. An empty `cfg.Subjects` (mirror / source-only streams) skips the check.
- **No other config drift reconciliation** — `MaxBytes`, `MaxAge`, `Retention`, `Storage`, KV bucket config, consumer config, etc. are NOT reconciled through `Ensure*`. Callers needing drift correction for those fields must issue an explicit `js.UpdateStream` / `js.CreateOrUpdateKeyValue` outside the helper.
- **Stuck-resource recovery (method form by default)** — the lookup-first fast path normally trusts per-stream Raft to settle on its own when an existing resource is not yet publishable. A crashed-peer scenario can leave a resource in a state Raft cannot recover from without external nudging, in which case the loop would spin until ctx expires. When `WithStuckResourceRecovery(after)` is set (and the stuck-existing observation persists for `after`), the helper re-issues `CreateOrUpdateStream` / `CreateOrUpdateKeyValue` with the requested cfg as an idempotent nudge; errors are logged but never abort the wait. The method-form helpers enable this by default with `DefaultStuckResourceRecoveryThreshold` (30s); package-level callers must opt in. Set `WithStuckResourceRecovery(0)` to disable.
...
| `WithStuckResourceRecovery(after time.Duration)` | After a continuous stuck-existing observation has persisted for `after`, re-issue `CreateOrUpdate*` as an idempotent nudge for resources Raft cannot recover from on its own. Method-form helpers enable this by default with `DefaultStuckResourceRecoveryThreshold` (30s); package-level callers must opt in. `0` disables. Errors from the recovery call are logged but never abort the wait. |
